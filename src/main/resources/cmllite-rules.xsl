    ...
    <xsl:if test="parent::cml:molecule">
      <xsl:if test="not(@count)">
        <o:error>
          ...
          child molecules must have a count specified</o:error>
      </xsl:if>
    </xsl:if>
    <xsl:apply-templates mode="cmllite" />
  </xsl:template>
  ...
  <xsl:template match="cml:atom" mode="cmllite">
    ...
    <xsl:if test="not(@id) and ../../cml:formula">
      <svrl:failed-assert test="not(@id) and ../../cml:formula">
        ...
        <svrl:text> atoms must have id unless they are part of an
          atomArray in a formula</svrl:text>
      </svrl:failed-assert>
    </xsl:if>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        >
          <xsl:attribute name="location"><xsl:apply-templates
            select="." mode="schematron-get-full-path" /></xsl:attribute>
          <svrl:text>
