...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template
        ...
        <xsl:choose>
            <xsl:when test="self::cml:cml">
                <xsl:choose>
                    <xsl:when test="not(cml:molecule)">
                        <!-- no molecule children -->
                        <xsl:call-template name="error">
                            <xsl:with-param name="location">
                                <xsl:apply-templates select="." mode="get-full-path"/>
                            </xsl:with-param>
                            <xsl:with-param name="text">there must be at least one "molecule" element as a child of the
                            ...
                            <xsl:otherwise>
                                ...
                                <xsl:choose>
                                    <xsl:when
                                        ...
                                        <!-- the molecules are in the molecular convention -->
                                        <xsl:apply-templates mode="molecular"/>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:call-template name="error">
                                            ...
                                            <xsl:with-param name="text">there must be at least one "molecule" element in
            ...
            <xsl:otherwise>
                <xsl:call-template name="error">
                    ...
                    <xsl:with-param name="text">the only valid cml elements which specify the molecular convention are
                        "cml" and "molecule"
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
                ...
                <xsl:call-template name="warning">
                    ...
                    <xsl:with-param name="text">the
                        <xsl:value-of select="$conventionName"/> convention does not include the element "<xsl:value-of
                                select="name(.)"/>". A different convention should be specified on this element
                    </xsl:with-param>
                </xsl:call-template>
                <xsl:apply-templates mode="molecular"/>
            </xsl:otherwise>
    ...
    <xsl:template match="cml:molecule" mode="molecular" name="molecule-template">
        ...
        <xsl:choose>
            <xsl:when test="@id">
                <xsl:if test="count(//cml:molecule[@id = current()/@id]) > 1">
                    <xsl:call-template name="error">
                        <xsl:with-param name="location">
                            <xsl:apply-templates select="." mode="get-full-path"/>
                        </xsl:with-param>
                        <xsl:with-param name="text">the id of a molecule must be unique within the document (duplicate
            ...
            <xsl:otherwise>
                <xsl:call-template name="error">
                    ...
                    <xsl:with-param name="text">molecules must have id specified</xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
        ...
        <xsl:if test="cml:molecule and cml:atomArray">
            <xsl:call-template name="error">
                ...
                <xsl:with-param name="text">molecules with child molecules must not contain an atomArray
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates mode="molecular"/>
    ...
    <xsl:template match="cml:atomArray" mode="molecular">
        ...
        <xsl:if test="count(../cml:atomArray) > 1">
            <xsl:call-template name="error">
                ...
                <xsl:with-param name="text">there should only be one atomArray child in a
    ...
    <xsl:template match="cml:bondArray" mode="molecular">
        ...
        <xsl:if test="count(../cml:bondArray) > 1">
            <xsl:call-template name="error">
                ...
                <xsl:with-param name="text">there should only be one bondArray child in a
                    <xsl:value-of select="name(..)"/> element
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template match="cml:bond" mode="molecular">
        ...
        <xsl:if test="not(ancestor::cml:molecule[1]//cml:atom[@id = substring-before(current()/@atomRefs2, ' ')])">
            <xsl:call-template name="error">
                ...
                <xsl:with-param name="text">the atoms in the atomRefs2 must be within the eldest containing molecule
                    (not found [<xsl:value-of select="substring-before(@atomRefs2, ' ')"/>])
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        ...
        <xsl:if test="not(ancestor::cml:molecule[1]//cml:atom[@id = substring-after(current()/@atomRefs2, ' ')])">
            <xsl:call-template name="error">
                ...
                <xsl:with-param name="text">the atoms in the atomRefs2 must be within the eldest containing molecule
                    (not found [<xsl:value-of select="substring-after(@atomRefs2, ' ')"/>])
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        ...
        <xsl:choose>
                ...
                <xsl:if test="@order/. = $other-value">
                    <xsl:if test="not(@dictRef)">
                        <xsl:call-template name="warning">
                            ...
                            <xsl:with-param name="text">it is recommended that bonds with order "other" have further
                                semantics given using the dictRef attribute
                            </xsl:with-param>
                        </xsl:call-template>
                    </xsl:if>
                </xsl:if>
            ...
            <xsl:otherwise>
                <xsl:call-template name="error">
                    ...
                    <xsl:with-param name="text">bonds must have the bond order specified using the "order" attribute
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="cml:bondStereo" mode="molecular">
        ...
        <xsl:if test="@atomRefs2">
            ...
            <xsl:if test="not(contains(@atomRefs2/., $parent-atom-1) and contains(@atomRefs2/., $parent-atom-2))">
                <xsl:call-template name="error">
                    ...
                    <xsl:with-param name="text">the atomRefs2 in a bondStereo must contain the atoms in the parent bond
                        (in any order)
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:if>
        ...
        <xsl:if test="@atomRefs4">
            ...
            <xsl:if test="not(contains(@atomRefs4/., $parent-atom-1) and contains(@atomRefs4/., $parent-atom-2))">
                <xsl:call-template name="error">
                    ...
                    <xsl:with-param name="text">the atomRefs4 in a bondStereo must contain the atoms in the parent bond
                        (in any order)
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:if>
        ...
        <xsl:if test=". = $other-value and not(@dictRef)">
            <xsl:call-template name="warning">
                ...
                <xsl:with-param name="text">a bondStereo of type "other" should use a dictRef to add further semantics
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates mode="molecular"/>
    ...
    <xsl:template match="cml:formula" mode="molecular">
            ...
            <xsl:when test="parent::cml:formula">
                <xsl:if test="not(@count)">
                    <xsl:call-template name="error">
                        ...
                        <xsl:with-param name="text">a formula that is a child of formula must have a count attribute
                            specified
                        </xsl:with-param>
                    </xsl:call-template>
                </xsl:if>
        ...
        <xsl:if test="not(cml:atomArray or @concise or @inline)">
            <xsl:call-template name="error">
                ...
                <xsl:with-param name="text">formula must have at least one of: an atomArray child, a concise attribute,
                    a inline attribute
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template match="cml:property" mode="molecular">
        ...
        <xsl:if test="not(@title)">
            <xsl:call-template name="warning">
                ...
                <xsl:with-param name="text">it is recommended that properties have a title attribute</xsl:with-param>
            </xsl:call-template>
        </xsl:if>
        <xsl:apply-templates mode="molecular"/>
    </xsl:template>
    ...
    <xsl:template name="check-all-atomRefs-values-are-different">
        ...
        <xsl:if test="not(count($atomRefTokens) = count(distinct-values($atomRefTokens)))">
            <xsl:call-template name="error">
                ...
                <xsl:with-param name="text">the atoms in an atomRefs<xsl:value-of select="count($atomRefTokens)"/> must
