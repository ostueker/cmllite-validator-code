...
<xsl:stylesheet version="2.0"
    ...
    <xsl:variable name="conventionName">unit</xsl:variable>
    ...
    <xsl:template match="*|@*|text()" mode="unit">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:call-template name="error">
                    ...
                    <xsl:with-param name="text">the only valid cml elements which can specify the unit convention is "unitList"</xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>

    <xsl:template match="cml:*" mode="unit">
        <xsl:call-template name="info">
            ...
            <xsl:with-param name="text">
                ...
                http://www.xml-cml.org/convention/unit convention
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>

    <xsl:template match="cml:unitList" mode="unit" name="unitList-template">
        <xsl:choose>
        ...
        <xsl:if test="not(@title)">
            <xsl:call-template name="warning">
                ...
                <xsl:with-param name="text">The unit element SHOULD have a title attribute intended for
                    human-readability
                </xsl:with-param>
            </xsl:call-template>
        </xsl:if>
    ...
    <xsl:template match="cml:unit" mode="unit">
    ...
    <xsl:template match="cml:definition" mode="unit">
    ...
    <xsl:template match="cml:description" mode="unit">
